...
public class CustomerAddEventTest {
    ...
    public CustomerEvent createEvent() {
    ...
    @Test
    public void CustomerOrgAddEventTest() throws Exception {
        Customer customerOrg = createCustomerOrg();

        CustomerEvent recordBefore = createEvent();
        Customer custAddedRecord = api.customer.addEvent(customerOrg, recordBefore);

        ArrayList<CustomerEvent> list = custAddedRecord.getEvents();

        CustomerEvent recordContained = list.get(0);
        Assert.assertTrue(recordBefore.getType().equals(recordContained.getType()));
    }
