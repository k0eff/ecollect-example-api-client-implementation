...
public class CustomerAddAddressTest {
    ...
    public Address createAddress() {
        ...
        addr.setAddress_lines(addrLnLst); // required field
        addr.setZip("40212");
        addr.setCity("Berlin"); // required field
        addr.setDistrict("Oberbilk");
        addr.setCountry(CountryCode.DE); // required field
        addr.setType(Address.TypeEnum.RESIDENCE);
        return addr;
    ...
    @Test
    public void CustomerOrgAddAddressTestReal() throws Exception {
        Customer customerOrg = createCustomerOrg();

        Address address = createAddress();
        Customer custAdded = api.customer.addAddress(customerOrg, address);

        ArrayList<Address> list = custAdded.getEntity().getAddresses();

        Address recordContained = list.get(1);
        Assert.assertTrue(address.getCity().equals(recordContained.getCity()));
    }
