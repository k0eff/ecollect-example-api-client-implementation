...
public class CustomerAddContactTest {
    ...
    @Test
    public void CustomerOrgAddContactTestReal() throws Exception {
        Customer customerOrg = createCustomerOrg();

        Contact contact = createContact();
        Customer custAddedContact = api.customer.addContact(customerOrg, contact);

        ArrayList<Contact> contactList = custAddedContact.getEntity().getContacts();

        Contact contactContained = contactList.get(0);
        Assert.assertTrue(contact.getValue().equals(contactContained.getValue()));
    }
