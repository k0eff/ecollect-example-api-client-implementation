...
public class InvoiceListTest {
    ...
    @Test
    public void InvoiceListTest() throws Exception {
        Invoice invoice = createInvoice();
        Invoice invoicePosted = postInvoice(invoice);
        Invoice invoicePosted2 = postInvoice(invoice);

        InvoiceList invList = api.invoice.getManyInvoices(2,0);
        Assert.assertTrue(invList.getData().size() == 2);
    }
}
